...
class Blocks:
    ...
    def _create_tables(self):
        ...
        # create index on IP (if not made by pk)
        cursor.execute('''
        ...
        cursor.execute('''
        CREATE INDEX IF NOT EXISTS idx_is_blocked ON blocks(is_blocked)
        ''')
    ...
    def get_current_blocks(self) -> list:
        """
        Get currently blocked IPs based on the is_blocked flag
    ...
    def get_active_blocks(self) -> list:
        """
        Get all currently active blocks that haven't expired yet
        
        Returns:
            List of active block records
        """
        try:
            cursor = self.conn.cursor()
            ...
            cursor.execute('''
            SELECT * FROM blocks
            WHERE is_blocked = 1 AND (unblock_time IS NULL OR unblock_time > ?)
            ''', (current_time,))
    ...
    def clean_expired_blocks(self) -> list:
        """
